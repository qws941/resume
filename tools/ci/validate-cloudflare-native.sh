...
if [[ ! -f "${ROOT_DIR}/apps/portfolio/wrangler.toml" ]]; then
...
naming_drift_matches="$(rg -n "\\bJOB_DASHBOARD\\b|\\bJOB_AUTOMATION_DB\\b|\\bJOB_CACHE\\b|\\bJOB_RATE_LIMIT\\b" \
  "${ROOT_DIR}/README.md" \
  "${ROOT_DIR}/docs/deployment-guide.md" \
  "${ROOT_DIR}/apps/job-server/AGENTS.md" \
  "${ROOT_DIR}/apps/job-dashboard/AGENTS.md" \
  "${ROOT_DIR}/apps/job-dashboard/README.md" 2>/dev/null || true)"
...
if [[ -n "${jsonc_doc_matches}" ]]; then
  ...
  echo "Expected active config path: apps/job-dashboard/wrangler.toml"
