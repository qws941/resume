export async function add_skill(params, sessionManager) {
  ...
  const result = await sessionManager.addResumeSkill(params.resume_id, params.tag_type_id);
  return {
    success: true,
...
export async function delete_skill(params, sessionManager) {
  ...
  await sessionManager.deleteResumeSkill(params.resume_id, params.skill_id);
  return {
    success: true,
