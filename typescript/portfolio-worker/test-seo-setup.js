...
async function validateSitemap() {
  ...
  try {
    ...
    urls.forEach(url => {
      ...
      else if (loc.includes('grafana') || loc.includes('gitlab') || loc.includes('n8n')) stats.externalServices++;
      else stats.sections++;
    });
    ...
    log(COLORS.green, '\n🌐 hreflang Coverage:');
    log(COLORS.blue, `  • URLs with hreflang: ${stats.withHreflang}/${urls.length} (${(stats.withHreflang/urls.length*100).toFixed(1)}%)`);
