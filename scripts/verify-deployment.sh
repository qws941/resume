...
if HTTP_STATUS=$(curl -sf -I "$SITE_URL/og-image.png" 2>/dev/null | head -1); then
    if echo "$HTTP_STATUS" | grep -q "200"; then
        echo -e "${GREEN}✓ HTTP 200 OK${NC}"

        # Download and check file type
        curl -sf "$SITE_URL/og-image.png" -o /tmp/og-test.png 2>/dev/null
        if FILE_TYPE=$(file /tmp/og-test.png 2>/dev/null); then
            if echo "$FILE_TYPE" | grep -q "PNG image data"; then
                SIZE=$(stat -c%s /tmp/og-test.png 2>/dev/null || echo "0")
                SIZE_KB=$(echo "scale=2; $SIZE / 1024" | bc)
                echo -e "${GREEN}✓ Valid PNG image (${SIZE_KB} KB)${NC}"

                # Check dimensions
                if command -v identify &> /dev/null; then
                    DIMS=$(identify -format "%wx%h" /tmp/og-test.png 2>/dev/null || echo "unknown")
                    if [[ "$DIMS" == "1200x630" ]]; then
                        echo -e "${GREEN}✓ Correct dimensions: $DIMS${NC}"
                        ((PASS++))
                    else
                        echo -e "${YELLOW}⚠ Dimensions: $DIMS (expected: 1200x630)${NC}"
                        ((WARN++))
                    fi
                else
                    echo -e "${GREEN}✓ File type validated (ImageMagick not available for size check)${NC}"
                    ((PASS++))
                fi
            else
