...
const VALID_RESUME_DATA = {
  ...
  education: {  // CORRECTED: Object instead of array
    school: 'University of Example',  // CORRECTED: school not school_name
    major: 'Computer Science'
  },
  summary: {  // CORRECTED: Object with totalExperience and expertise
    totalExperience: '5년',
  ...
  careers: [  // CORRECTED: Array items with period format
    {
      company: 'Example Corp',
  ...
  skills: {  // CORRECTED: Object with category keys
...
test('Validation Adapter - should import validation adapter successfully', async () => {
  ...
  strictEqual(typeof validation.validateResumeData, 'function', 'validateResumeData should be exported');
  strictEqual(typeof validation.formatErrorsForMCP, 'function', 'formatErrorsForMCP should be exported');
});
...
test('Validation Logic - should reject resume with wrong types', async () => {
  ...
  strictEqual(validation.errors.length > 0, true, 'Should have validation errors for type mismatches');
});
...
test('Resume-Sync Import - should have validation blocks in resume-sync.js', async () => {
  ...
  strictEqual(content.includes('validateResumeData'), true, 'Should import and use validateResumeData');
...
test('Resume-Sync Import - should have validation imports in resume-sync.js', async () => {
  ...
  strictEqual(content.includes("from '../shared/validation/index.js'"), true, 'Should import from validation module');
  strictEqual(content.includes('import { validateResumeData, formatErrorsForMCP, masterSchema }'), true, 'Should import validation functions');
});
...
test('Block Placement - should have export validation block', async () => {
  ...
  const exportMatch = content.match(/export\s*:\s*async\s*\([\s\S]*?\)\s*=>\s*\{[\s\S]*?validateResumeData[\s\S]*?\}/);
...
test('Block Placement - should have import validation block', async () => {
  ...
  const importMatch = content.match(/import\s*:\s*async\s*\([\s\S]*?\)\s*=>\s*\{[\s\S]*?validateResumeData[\s\S]*?\}/);
...
test('Block Placement - should have sync validation block', async () => {
  ...
  const syncMatch = content.match(/sync\s*:\s*async\s*\([\s\S]*?\)\s*=>\s*\{[\s\S]*?validateResumeData[\s\S]*?\}/);
...
test('Error Format Compliance - should return MCP-compliant error object for export', async () => {
  ...
  const mcpTools = resumeSync.default;
  const errorResponse = {
    success: false,
    error: 'Test error',
