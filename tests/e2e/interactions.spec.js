...
test.describe('Smooth Scroll Behavior', () => {
  ...
  test('logo click should navigate to homepage', async ({ page }) => {
    ...
    // Click logo - should navigate to homepage (href="/")
    const logo = page.locator('.nav-logo');
    const href = await logo.getAttribute('href');
    expect(href === '/' || href === '#').toBeTruthy();

    // Click logo and wait for any scroll/navigation
    await logo.click();
    await page.waitForTimeout(600);

    // Logo should be visible and functional
...
test.describe('Navigation Scroll Effect', () => {
  test('nav should get shadow on scroll', async ({ page }) => {
    ...
    await page.waitForTimeout(500); // Wait for transition (0.3s)
...
test.describe('Link Hover States', () => {
  test('nav links should have hover effect', async ({ page }) => {
    ...
    const cursor = await navLink.evaluate(
  ...
  test('project card links should be clickable', async ({ page }) => {
    ...
    const cursor = await firstLink.evaluate(
      (el) => window.getComputedStyle(el).cursor,
    );
    expect(cursor).toBe('pointer');
  });
...
test.describe('URL Hash Navigation', () => {
  test('should scroll to resume section on hash navigation', async ({
    page,
  }) => {
  ...
  test('should scroll to projects section on hash navigation', async ({
    page,
  }) => {
  ...
  test('should scroll to contact section on hash navigation', async ({
    page,
  }) => {
