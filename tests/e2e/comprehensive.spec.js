...
const EXPECTED = {
  ...
  NAV_LINKS: 5, // 경력, 프로젝트, 기술, 자격증, 연락처
  CONTACT_ITEMS: 5, // Email, Phone, GitHub, Website, Monitoring
  DOWNLOAD_LINKS: 3, // PDF, DOCX, Markdown
};
...
test.describe('Navigation', () => {
  ...
  test('should have all navigation links', async ({ page }) => {
    ...
    // Verify nav links by href (locale-agnostic)
    ...
    await expect(navLinks.nth(2)).toHaveAttribute('data-i18n', 'nav.skills');

    await expect(navLinks.nth(3)).toHaveAttribute('href', '#certifications');
    ...
    await expect(navLinks.nth(4)).toHaveAttribute('data-i18n', 'nav.contact');
  });
...
test.describe('Hero Section', () => {
  ...
  test('should display hero title', async ({ page }) => {
    ...
    await expect(title).toContainText('Senior DevSecOps Engineer');
  });
  ...
  test('should have download links', async ({ page }) => {
    ...
    // DOCX download link
    const docxLink = downloadLinks.nth(1);
    await expect(docxLink).toContainText('DOCX');

    // Markdown download link
    const mdLink = downloadLinks.nth(2);
    await expect(mdLink).toContainText('Markdown');
  });
});
